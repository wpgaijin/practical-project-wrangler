...
if( !class_exists( 'PPW_Metabox_Clients_General' ) ) {
  class PPW_Metabox_Clients_General {
    /**
     * Metabox ID
     *
     * @since  0.0.1
    ...
    public function __construct() {
      $this->id        = apply_filters( 'ppw_metabox_clients_general_id', PPW_PREFIX . '_clients_general_info' );
      $this->name      = apply_filters( 'ppw_metabox_clients_general_name', 'General Information' );
      $this->post_type = apply_filters( 'ppw_metabox_clients_general_post_type', 'ppw_clients' );
      $this->context   = apply_filters( 'ppw_metabox_clients_general_context', 'normal' );
      $this->priority  = apply_filters( 'ppw_metabox_clients_general_priority', 'high' );
    ...
    public function save( $post_id  ) {
      $fields = $this->fields();
      $nonce = PPW_PREFIX . '_clients_general_nonce';
      ...
      foreach( $fields as $field ) {
        if ( !empty( $_POST[ $field ] ) ) {
          $new = apply_filters( 'ppw_mb_clients_general_save_' . $field, sanitize_text_field( $_POST[ $field ] ) );
          update_post_meta( $post_id, $field, $new );
        } else {
          delete_post_meta( $post_id, $field );
        }
      }
      do_action( 'ppw_save_clients_general', $post_id, $post );
    } // end save
  }
} // end PPW_Metabox_Clients_General